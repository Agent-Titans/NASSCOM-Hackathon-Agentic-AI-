...
# Run evaluation (same logic as synthetic_eval)
with Session() as session:
    ...
    # Cost estimate assumptions used for report benchmarking
    USD_TO_INR = 83.0
    ASSUMED_COST_PER_SECOND_USD = 0.003  # conservative per-second processing estimate
    estimated_cost_usd = avg_latency * ASSUMED_COST_PER_SECOND_USD
    estimated_cost_inr = estimated_cost_usd * USD_TO_INR
    estimated_cost_paisa = estimated_cost_inr * 100
    ...
    metrics = {
        ...
        'per_ticket_cost_inr': estimated_cost_inr,
        'per_ticket_cost_usd': estimated_cost_usd,
        'per_ticket_cost_paisa': estimated_cost_paisa,
        'per_label': {label: {'count': counts[label], 'prec': precision[label], 'rec': recall[label], 'f1': f1[label]} for label in dept_labels},
    }

    # generate PDF
    out_path = os.path.join('tmp', 'synthetic_report.pdf')
    doc = SimpleDocTemplate(out_path, pagesize=letter)
    styles = getSampleStyleSheet()
    styles.add(ParagraphStyle(name='TitleColor', parent=styles['Title'], textColor=colors.HexColor('#0b3d91')))
    styles.add(ParagraphStyle(name='SectionHeader', parent=styles['Heading2'], textColor=colors.HexColor('#0b3d91')))
    styles.add(ParagraphStyle(name='SummaryBullet', parent=styles['Bullet'], leftIndent=12, bulletIndent=0, textColor=colors.HexColor('#333333')))
    story = []

    story.append(Paragraph('Synthetic Evaluation Report', styles['TitleColor']))
    story.append(Paragraph(f'Date: {datetime.utcnow().isoformat()} UTC', styles['Normal']))
    ...
    exec_lines = []
    exec_lines.append(
        f"This synthetic evaluation of {metrics['total']} tickets shows an overall accuracy of {metrics['accuracy']:.2%}, micro-F1 {metrics['micro_f1']:.2%}, and macro-F1 {metrics['macro_f1']:.2%}."
    )
    exec_lines.append(
        f"Average pipeline latency per ticket was {metrics['avg_latency']:.3f} seconds and the observed deflection rate was {metrics['deflection_rate']:.2%}."
    )
    exec_lines.append(
        f"Estimated per-ticket processing cost is INR {metrics['per_ticket_cost_inr']:.3f} ({metrics['per_ticket_cost_paisa']:.1f} paisa), based on an assumed nominal compute cost of USD {ASSUMED_COST_PER_SECOND_USD:.4f}/sec."
    )
    exec_lines.append(
        'Primary strengths: high precision for Storage and SecOps categories; areas for improvement: Software and Network routing recall.'
    )
    exec_lines.append(
        'Recommendation: improve training/examples for Software/Network use-cases, and consider tuned keyword fallbacks or RAG growth for low-grounding categories to increase recall.'
    )
    ...
    summary = [
        f"Total synthetic tickets: {metrics['total']}",
        f"Accuracy: {metrics['accuracy']:.4f}",
        f"Micro F1: {metrics['micro_f1']:.4f}",
        f"Macro F1: {metrics['macro_f1']:.4f}",
        f"Deflection Rate: {metrics['deflection_rate']:.4f}",
        f"Average latency (s): {metrics['avg_latency']:.4f}",
        f"Estimated cost per ticket: INR {metrics['per_ticket_cost_inr']:.3f} ({metrics['per_ticket_cost_paisa']:.1f} paisa)",
    ]
    for s in summary:
    ...
    table_data = [
        ...
        ['Deflection Rate', f"{metrics['deflection_rate']:.4f}"],
        ['Avg Latency (s)', f"{metrics['avg_latency']:.4f}"],
        ['Estimated cost per ticket (INR)', f"{metrics['per_ticket_cost_inr']:.3f}"],
        ['Estimated cost per ticket (paisa)', f"{metrics['per_ticket_cost_paisa']:.1f}"],
    ]
    ...
    # per-category table
    per_table = [['Department', 'Count', 'Precision', 'Recall', 'F1']]
    for label, vals in metrics['per_label'].items():
        per_table.append([label, str(vals['count']), f"{vals['prec']:.4f}", f"{vals['rec']:.4f}", f"{vals['f1']:.4f}"])
    pt = Table(per_table, hAlign='LEFT')
    ...
    story.append(Paragraph('Per-department performance', styles['SectionHeader']))
    story.append(pt)
