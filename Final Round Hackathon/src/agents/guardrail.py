...
class GuardrailAI:
    ...
    def _layer2_llm_scan(self, redacted_text: str) -> None:
        ...
        Fail-closed: if the guardrail API errors, we still halt to Hand 3.
        """
        ...
        if verdict != "SECURITY_OK":
            # API down, malformed output, or ambiguous — safe default per LLD.
            raise SecurityGuardrailException(
                f"guardrail_scan_inconclusive:{verdict}",
                layer="llm",
            )
