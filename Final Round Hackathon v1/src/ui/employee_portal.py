...
def render_portal_create(user: User, session) -> None:
    ...
    st.markdown(
        _wrap(
            ...
            '<p class="portal-create-title">Submit a request</p>'
            '<p class="portal-create-sub">Tell us what went wrong — we will route it to the right team.</p>'
            "</div>"
        ),
        unsafe_allow_html=True,
    )
    ...
    with st.form("portal_create_form", border=False):
        st.markdown(
            _wrap('<p class="portal-form-section">What is the issue?</p>'),
            unsafe_allow_html=True,
        )
        title = st.text_input(
            "Issue title",
        ...
        st.markdown(
            _wrap('<p class="portal-form-section">Category</p>'),
            unsafe_allow_html=True,
        )
        category = st.radio(
            "Category",
        ...
        st.markdown(
            _wrap('<p class="portal-form-section">Quick details (optional)</p>'),
            unsafe_allow_html=True,
        )
        sc1, sc2 = st.columns(2)
        symptoms: list[str] = []
        with sc1:
            if st.checkbox("Cannot access system or app", key="sym_access"):
                symptoms.append(_SYMPTOM_OPTIONS[0])
            if st.checkbox("Error message showing", key="sym_error"):
                symptoms.append(_SYMPTOM_OPTIONS[1])
        with sc2:
            if st.checkbox("Started today", key="sym_today"):
                symptoms.append(_SYMPTOM_OPTIONS[2])
            if st.checkbox("Tried restarting", key="sym_restart"):
                symptoms.append(_SYMPTOM_OPTIONS[3])

        st.markdown(
            _wrap('<p class="portal-form-section">Describe what happened</p>'),
            unsafe_allow_html=True,
        )
        description = st.text_area(
            "Description",
            height=120,
            placeholder="What happened? Include timing, error text, and anything you already tried.",
            label_visibility="collapsed",
        )
    ...
    if ticket.hand == "3":
        ...
        with fb2:
            if st.button("Did not work", key=f"portal_fb_failed_{ticket_id}", use_container_width=True):
                ...
                st.session_state["portal_flash"] = "Sent to a specialist for follow-up."
                st.session_state["portal_flash_type"] = "human"
                st.session_state["portal_view"] = "home"
                st.rerun()
