#!/usr/bin/env python3
"""
Final50 — 50 unique final evaluation tickets (live routing + full report).

  python scripts/final50_assessment.py --live --fresh --delay 2.5

Output:
  docs/final50_results.json
  docs/FINAL50_SELF_EVALUATION.md
  test-reports/final50_report.html
"""
...
SCENARIOS = ROOT / "data" / "set_final50_scenarios.json"
RESULTS = ROOT / "docs" / "final50_results.json"
REPORT_MD = ROOT / "docs" / "FINAL50_SELF_EVALUATION.md"
REPORT_HTML = ROOT / "test-reports" / "final50_report.html"
...
def run_live(*, fresh: bool = False, delay: float = 2.0) -> dict[str, Any]:
    ...
    print(f"=== Final50: {len(pending)} pending / {len(specs)} ===", flush=True)
...
def _llm_judge_final50(cases: list[dict], summary: dict, f1: dict, arch: dict) -> dict[str, Any]:
    from src.clients.gemini_client import GeminiClient
...
Suite: Final50 — 50 unique live-routed tickets (Amazon, Google, Deloitte, Nike, Tesla).
...
{{"overall":0-10,"verdict":"one sentence","use_case_alignment":0-10,"lld_alignment":0-10,"responsible_ai":0-10,"ethical_ai":0-10,"security_posture":0-10,"evaluation_rigor":0-10,"strengths":["",""],"improvements":["",""]}}"""
    ...
    if not client.available:
        return {
            ...
            "verdict": f"Final50: {int(rate*100)}% routing pass, F1 {f1.get('macro_f1')}, five-firm enterprise suite.",
...
def _render_markdown(data: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Gemini API usage",
            "",
            f"- **Tickets with Gemini:** {gem.get('tickets_with_gemini', 0)}/50",
            f"- **Model usage:** {gem.get('model_usage', {})}",
            "",
...
def _render_html(data: dict[str, Any]) -> None:
    ...
    case_rows = "".join(
        ...
        f'<td>{c.get("latency_sec")}s</td></tr>'
        for c in cases
    )

    doc = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>SAARTHI Final50 Report</title>
<style>
...
<div class="hero"><h1>SAARTHI Final50 — Final Evaluation Report</h1>
<p>50 unique tickets · Amazon · Google · Deloitte · Nike · Tesla · F1 · LLM jury · UI smoke</p>
<p>{ts}</p></div>
<div class="stats">
...
<div class="stat"><div class="num">{data.get('gemini', {}).get('tickets_with_gemini', 0)}</div>Gemini tickets</div>
<div class="stat"><div class="num">{'PASS' if data.get('ui', {}).get('ok') else 'FAIL'}</div>UI smoke</div>
</div>
<p>Gemini models: {data.get('gemini', {}).get('model_usage', {})}</p>
...
<div class="card"><h2>All 50 final evaluation tickets</h2>
<table><thead><tr><th>ID</th><th>Title</th><th>Exp dept</th><th>Act dept</th><th>Hand</th><th>Clf</th><th>Gemini</th><th>Models</th><th>Lat</th></tr></thead>
<tbody>{case_rows}</tbody></table></div>
<p style="color:#64748b;font-size:.75rem">data/set_final50_scenarios.json · scripts/final50_assessment.py</p>
</body></html>"""
...
def main() -> int:
    p = argparse.ArgumentParser(description="SAARTHI Final50 evaluation")
    ...
    llm = _llm_judge_final50(cases, summary, f1, arch)
