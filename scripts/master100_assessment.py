#!/usr/bin/env python3
"""
Master100 — 100 Nextera Technologies master evaluation tickets (live routing + full report).

  python scripts/master100_assessment.py --live --fresh --delay 4.0

Output:
  docs/master100_results.json
  docs/MASTER100_SELF_EVALUATION.md
  test-reports/master100_report.html
"""
...
RESULTS = ROOT / "docs" / "master100_results.json"
REPORT_MD = ROOT / "docs" / "MASTER100_SELF_EVALUATION.md"
REPORT_HTML = ROOT / "test-reports" / "master100_report.html"
...
def run_live(*, fresh: bool = False, delay: float = 2.0) -> dict[str, Any]:
    ...
    print(f"=== Master100: {len(pending)} pending / {len(specs)} ===", flush=True)
...
def _llm_judge_master100(cases: list[dict], summary: dict, f1: dict, arch: dict) -> dict[str, Any]:
    from src.clients.gemini_client import GeminiClient
...
Suite: Master100 — 100 live-routed tickets for Nextera Technologies (global technology firm).
...
{{"overall":0-10,"verdict":"one sentence","use_case_alignment":0-10,"lld_alignment":0-10,"responsible_ai":0-10,"ethical_ai":0-10,"security_posture":0-10,"evaluation_rigor":0-10,"strengths":["",""],"improvements":["",""]}}"""
    ...
    if not client.available:
        return {
            ...
            "verdict": f"Master100: {int(rate*100)}% routing pass, F1 {f1.get('macro_f1')}, 100-ticket master jury suite.",
...
def _render_markdown(data: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Gemini API usage",
            "",
            f"- **Tickets with Gemini:** {gem.get('tickets_with_gemini', 0)}/100",
...
def _render_html(data: dict[str, Any]) -> None:
    ...
    doc = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>SAARTHI Master100 Report</title>
<style>
...
<div class="hero"><h1>SAARTHI Master100 — Final Evaluation Report</h1>
<p>100 tickets · Nextera Technologies · F1 · LLM jury · UI smoke</p>
<p>{ts}</p></div>
<div class="stats">
...
<div class="card"><h2>All 100 master evaluation tickets</h2>
...
<p style="color:#64748b;font-size:.75rem">data/set_master100_scenarios.json · scripts/master100_assessment.py</p>
</body></html>"""
...
def main() -> int:
    p = argparse.ArgumentParser(description="SAARTHI Master100 evaluation")
    ...
    llm = _llm_judge_master100(cases, summary, f1, arch)
